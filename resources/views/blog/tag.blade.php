            ...
            <nav class="flex justify-center text-sm text-slate-500 mb-6" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li>
                        <div class="flex items-center">
                            <span class="material-symbols-outlined text-[16px] mx-1">chevron_right</span>
                            <span class="text-charcoal font-semibold">#{{ $tag->name }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h1 class="font-display font-light text-4xl lg:text-5xl text-charcoal mb-4">#{{ $tag->name }}</h1>
            <p class="text-slate-500 max-w-2xl mx-auto">Showing all published articles tagged with #{{ $tag->name }}.</p>
        </div>
            ...
            @empty
                <div class="col-span-full py-20 text-center">
                    <div class="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4">
                        <span class="material-symbols-outlined text-4xl text-slate-400">tag</span>
                    </div>
                    <h3 class="text-xl font-bold text-charcoal mb-2">No articles found</h3>
                    <p class="text-slate-500">There are currently no published articles with this tag.</p>
                </div>
            @endforelse
        </div>

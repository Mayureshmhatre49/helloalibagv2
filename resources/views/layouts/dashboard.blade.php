    ...
    <div class="flex min-h-screen">
        ...
        <aside class="fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-border-light transform transition-transform duration-200 md:translate-x-0"
            ...
            <div class="flex flex-col h-full">
                {{-- Logo --}}
                <div class="flex items-center gap-3 px-5 h-16 border-b border-border-light flex-shrink-0">
                ...
                <nav class="flex-1 px-3 py-4 space-y-1">
                    ...
                    <a href="{{ route('owner.bookings.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium {{ request()->routeIs('owner.bookings.*') ? 'bg-primary/10 text-primary' : 'text-text-secondary hover:bg-background-light' }}" id="tour-bookings">
                        <span class="material-symbols-outlined text-[20px]">book_online</span>
                        Bookings
                        @php $pendingBookings = \App\Models\Booking::whereIn('listing_id', auth()->user()->listings()->pluck('id'))->pending()->count(); @endphp
                        @if($pendingBookings > 0)
                            <span class="ml-auto bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full font-bold">{{ $pendingBookings }}</span>
                        @endif
                    </a>
                    <a href="{{ route('owner.support.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium {{ request()->routeIs('owner.support.*') ? 'bg-primary/10 text-primary' : 'text-text-secondary hover:bg-background-light' }}" id="tour-support">

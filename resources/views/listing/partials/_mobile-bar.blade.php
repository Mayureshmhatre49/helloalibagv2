...
<div class="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-3 sm:px-4 py-3 z-40 shadow-[0_-4px_24px_rgba(0,0,0,0.08)] pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
    <div class="flex items-center gap-2 sm:gap-3">
        <div class="min-w-0 flex-1">
            ...
            @else
                <span class="text-sm font-semibold text-slate-700 truncate block">{{ $mobileBarLabel ?? 'Contact for pricing' }}</span>
            @endif
            @if($avgRating > 0)
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            ...
            <button onclick="document.getElementById('inquiry-section').scrollIntoView({behavior: 'smooth'})"
                    class="bg-primary hover:bg-primary/90 text-white font-bold px-4 sm:px-5 py-2.5 rounded-xl text-sm transition-colors shadow-sm shadow-primary/20 whitespace-nowrap">

...
@section('page-title', 'Admin: Edit Listing')
...
<div class="max-w-6xl mx-auto" x-data="{
    ...
    init() {
        ...
        var saved = localStorage.getItem('adminEditTab_{{ $listing->id }}');
        this.activeTab = this.errorTab || saved || 'basic';
        this.$watch('activeTab', v => localStorage.setItem('adminEditTab_{{ $listing->id }}', v));
    ...
    <div class="mb-4 flex items-start justify-between gap-4 flex-wrap">
        <div>
            <div class="flex items-center gap-2 mb-1">
                ...
                @endphp
                <span class="text-xs font-bold px-2.5 py-1 rounded-full {{ $statusColors[$listing->status] ?? 'bg-gray-100 text-gray-600' }}">
                    {{ ucfirst($listing->status) }}
                </span>
            </div>
            <p class="text-slate-500 text-sm ml-8">Owner: <span class="font-medium">{{ $listing->creator->name }}</span> &nbsp;·&nbsp; "{{ $listing->title }}"</p>
        </div>
    ...
    <div class="mb-5 flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800">
        <span class="material-symbols-outlined text-[20px] text-amber-500 flex-shrink-0 mt-0.5">admin_panel_settings</span>
        <div>
            <span class="font-bold">Admin Edit</span> — Your changes will be saved immediately without resetting the listing status or sending the listing back to the approval queue.
        </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row gap-8">
        ...
        <div class="flex-1 min-w-0">
            <form action="{{ route('admin.listings.update', ['listing' => $listing->slug]) }}" method="POST" enctype="multipart/form-data" id="editForm">
                @csrf
                @method('PUT')
                ...
                <div class="sticky bottom-0 z-40 bg-white border-t border-slate-200 p-4 mt-8 flex items-center justify-between rounded-b-2xl shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
                    ...
                    <div class="flex items-center gap-3 w-full sm:w-auto">
                        <a href="{{ route('admin.listings.index') }}" class="flex-1 sm:flex-none text-center px-6 py-3 rounded-xl font-bold text-sm text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors">Cancel</a>

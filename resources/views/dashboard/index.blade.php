...
@else
    ...
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl border border-border-light p-5">
            <div class="flex items-center justify-between mb-3">
                <span class="material-symbols-outlined text-green-600 bg-green-50 p-2 rounded-xl">book_online</span>
                @if($pendingBookings > 0)
                    <span class="bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full font-bold">{{ $pendingBookings }} new</span>
                @endif
            </div>
            <a href="{{ route('owner.bookings.index') }}" class="block">
                <p class="text-2xl font-bold text-text-main">{{ $pendingBookings }}</p>
                <p class="text-sm text-text-secondary">Pending Bookings</p>
            </a>
        </div>
    </div>

    {{-- Analytics Chart --}}
    <div class="bg-white rounded-2xl border border-border-light p-6 mb-6" id="tour-analytics">
    ...
    new Chart(canvas, {
        ...
        data: {
            ...
            datasets: [{
                ...
                fill: true,
                borderColor: '#1183d4',
                backgroundColor: 'rgba(17, 131, 212, 0.07)',
                borderWidth: 2,
                pointRadius: 2,
                pointHoverRadius: 5,
                tension: 0.4,
        ...
        options: {
            ...
            scales: {
                x: { grid: { display: false }, ticks: { maxRotation: 0, maxTicksLimit: 6, font: { size: 10 } } },
